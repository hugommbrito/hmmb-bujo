...
- [x] **Task 1: Reorganizar `RecurringTemplateManager` — abas por grupo + filtro "mostrar inativos"** (AC: #2)
  - [x] 1.1 Em `frontend/src/features/bujo/components/RecurringTemplateManager.tsx`, adicionar estado `const [group, setGroup] = useState<RecurrenceGroup>('weekly')` e `const [showInactive, setShowInactive] = useState(false)`.
  - [x] 1.2 Renderizar `<Tabs value={group} onChange={(_, v) => setGroup(v)} aria-label="Grupo de recorrência">` com três `<Tab value="weekly" label="Semanal" />` / `"monthly"` / `"annual"` — reutilizar `RECURRENCE_GROUP_LABEL` (já definido no arquivo) para os rótulos. **Primeira ocorrência de `Tabs` no projeto** (confirmado por grep) — importar de `@mui/material`; MUI 6.1 já gera o `role="tablist"`/`role="tab"`/`aria-selected` corretos. Passar `aria-label` no `<Tabs>` é obrigatório (baseline WCAG 2.2 AA, Story 2.4).
...
- [x] **Task 4: Testes** (AC: #1, #2)
  ...
  - [x] 4.2 Criar `frontend/src/pages/planner/RecurringPage.test.tsx` (smoke de composição, molde de `SettingsPage.test.tsx` que está sendo apagado): mockar o barrel `../../features/bujo` com `RecurringTemplateManager: () => <div data-testid="recurring-template-manager" />`; assertar que o `<main aria-label="Recorrentes">` existe, que o título "Recorrentes" (h5) renderiza, e que o manager é montado; `jest-axe` sem violações.
  - [x] 4.3 Atualizar `frontend/src/app/layout/Sidebar.test.tsx`: adicionar asserção de que o item "Recorrentes" aparece sob o grupo Planner expandido (junto de "Esta Semana"/"Este Mês"/"Futuro"). Conferir que os testes existentes de expandir/colapsar o grupo Planner continuam passando (o novo item entra no mesmo `Collapse`).
  - [x] 4.4 **Mocks de barrel** (achado recorrente 4.3/4.4/4.5): `router.test.tsx` (linha 33) e `RouteAnnouncer.test.tsx` (linha 30) mockam `features/bujo` com `RecurringTemplateManager: () => null`. Esse export **continua existindo** (agora consumido por `RecurringPage` em vez de `SettingsPage`), então o mock permanece válido — **não remover**. Rodar os dois testes para confirmar que a mudança de rota (`settings` → placeholder, nova rota `planner/recurring`) não os quebra; se `router.test.tsx` asseverar algo sobre a rota `settings`/`SettingsPage`, ajustar (grep confirmou que hoje não há asserção sobre settings, mas revalide após a mudança).
  - [x] 4.5 `grep -rn "SettingsPage" frontend/src` após apagar os arquivos — deve retornar **zero** ocorrências (garante que nenhum import órfão sobrou e o build/typecheck não quebra).

- [x] **Task 5: Verificação final** (AC: #1, #2)
  - [x] 5.1 `cd frontend && npm run typecheck && npm run lint && npm run build && npm run test` — colar a contagem **real** observada (guardrail retro Epic 3 §1; a suíte estava em 365 passed / 41 arquivos ao fim da 4.5 — o número muda: `SettingsPage.test.tsx` sai, `RecurringPage.test.tsx` entra, `RecurringTemplateManager.test.tsx` é reescrito, `Sidebar.test.tsx` ganha caso).
  - [x] 5.2 **Backend não roda nesta story** (nenhum arquivo `backend/` tocado). Não é necessário `uv run pytest`; se por acaso algum arquivo backend aparecer no `git status`, é bug — reverter.
  - [x] 5.3 **Verificação manual contra backend+frontend reais** (`npm run dev` + backend, logado): navegar por Planner → "Recorrentes" (novo item na sidebar). Confirmar: (a) o CRUD funciona ali (criar template `weekly` na aba Semanal, `monthly` na aba Mensal → cada um nasce no grupo certo); (b) trocar de aba mostra só os templates daquele grupo; (c) desativar um template o some da view com "mostrar inativos" desligado, e ligar o Switch o traz de volta com "(inativo)"; (d) ir em Configurações → agora é o placeholder (sem CRUD de recorrentes); (e) as seções de placement em "Esta Semana"/"Este Mês" continuam funcionando (regressão — não foram tocadas, mas confirmar que ainda listam ativos e colocam). Zero erros de console.
  - [x] 5.4 **File List por último** (retro Epic 3 §8-2, guardrail ativo): `git status --short` + `git diff --stat` **depois** da verificação manual, reconciliar contra o File List documentado. Confirmar que `pages/settings/` foi de fato removido (aparece como deleção no git).
...
- **Node 18 vs 22:** o shell padrão estava em Node 18, e `vitest`/`rolldown` exigem `styleText` de `node:util` (Node 20.12+). Rodar toda a suíte sob `nvm use 22` (v22.15.1). Sem isso, o runner morre no startup com `SyntaxError: ... does not provide an export named 'styleText'`.
- **Flakiness de paralelismo no vitest:** a suíte cheia com paralelismo default deu timeouts de 5000ms não-determinísticos (1ª run 7 falhas/6 arquivos; 2ª run 1 falha diferente) — máquina sobrecarregada (fase de import ~342s). Rodar com `--no-file-parallelism` deu run limpo e determinístico: **42 arquivos / 385 testes passando**. As falhas eram ambientais, não de asserção.
- **E2E cold-start Neon (branch `e2e`):** `recurring-templates.spec.ts` falhou em `fullyParallel`/serial quando rodou contra a branch Neon `e2e` fria — o round-trip de criação (POST → invalidate → GET list) e até o signup do fixture estouravam os 10s sob cold-start (fricção já documentada no `playwright.config.ts`). Subindo backend+vite manualmente e aquecendo o backend (health 2ms), a Playwright reusa os servidores (`reuseExistingServer` local) e **os 2 testes passam** (38s / 35s). Confirmado: falha era latência, não bug.
- **Gap de spec E2E não previsto pela story:** a story afirmava "não toca E2E specs", mas existe `frontend/e2e/recurring-templates.spec.ts` (permanente, adicionado por QA depois da 4.5) que dirigia a UI ANTIGA — clicava em "Configurações", esperava o form ali, e usava o `<Select>` "Grupo de recorrência" (todos removidos por esta story). Deixá-lo quebrado violaria o gate de regressão. Atualizado para a nova UI (navegar por "Recorrentes", abas em vez de Select, e o filtro `active`-only default: desativar some da view até ligar "Mostrar inativos"). Verificado passando contra o backend real.

### Completion Notes List

Story de **movimentação + apresentação** (frontend-only, zero arquivos `backend/` tocados — confirmado por `git status`). Nenhum endpoint/model/serializer/migração/contrato mudou.

- **AC1 (CRUD migra p/ o Planner):** `RecurringTemplateManager` agora é composto por `RecurringPage` (`/planner/recurring`), com item "Recorrentes" (`EventRepeatIcon`) no grupo Planner da Sidebar. `SettingsPage.tsx`/`.test.tsx` apagados (dir `pages/settings/` removido); rota `settings` revertida a `PlaceholderPage`. `grep -rn "SettingsPage" frontend/src` → zero.
- **AC2 (abas + filtro):** `Tabs`/`Tab` do MUI (primeira adoção no projeto, `aria-label="Grupo de recorrência"`, WCAG 2.2 AA); filtragem client-side por `group` + `showInactive` num único `useRecurringTemplatesQuery()` (sem params — a query key que as mutations invalidam por prefixo). Form segue a aba ativa como `recurrenceGroup` (o `<Select>` de grupo foi removido). `Switch` "Mostrar inativos" (default `false` = só ativos). Título único agora vive só na página (heading interno do manager removido).
- **Testes:** `RecurringTemplateManager.test.tsx` reescrito p/ o novo layout (abas, filtro, criação por-grupo, edição/toggle preservados, `jest-axe` contra o componente real). `RecurringPage.test.tsx` novo (smoke de composição). `Sidebar.test.tsx` ganha caso do item "Recorrentes" sob Planner. Mocks de barrel (`router.test.tsx`/`RouteAnnouncer.test.tsx`) seguem válidos (`RecurringTemplateManager` ainda é exportado) — revalidados.
- **Verificação (contagem real, guardrail Epic 3 §1):** `npm run typecheck` ✓, `npm run lint` ✓, `npm run build` ✓, `npm run test` → **392 passed / 42 files** (rodado com `--no-file-parallelism` p/ evitar flakiness ambiental de timeout).
- **Verificação E2E real (Task 5.3, contra backend+frontend de verdade):** `recurring-templates.spec.ts` (atualizado) — 2/2 passando contra a branch Neon `e2e`: criar weekly na aba Semanal e monthly na aba Mensal (cada um no grupo certo), troca de aba segmenta a lista, desativar some da view e "Mostrar inativos" traz de volta com "(inativo)", placement chega ao Weekly/Monthly Log, e independência instância/template (AC3). Zero erros de console.

### File List

- `frontend/src/features/bujo/components/RecurringTemplateManager.tsx` (modificado — abas + filtro; heading interno removido; `<Select>` de grupo removido)
- `frontend/src/features/bujo/components/RecurringTemplateManager.test.tsx` (reescrito p/ o novo layout)
- `frontend/src/pages/planner/RecurringPage.tsx` (novo)
- `frontend/src/pages/planner/RecurringPage.test.tsx` (novo)
- `frontend/src/app/router.tsx` (modificado — rota `planner/recurring` nova; `settings` revertida a `PlaceholderPage`)
- `frontend/src/app/layout/Sidebar.tsx` (modificado — item "Recorrentes" no grupo Planner; import `EventRepeatIcon`)
- `frontend/src/app/layout/Sidebar.test.tsx` (modificado — caso do item "Recorrentes")
- `frontend/src/pages/settings/SettingsPage.tsx` (apagado)
- `frontend/src/pages/settings/SettingsPage.test.tsx` (apagado)
- `frontend/e2e/recurring-templates.spec.ts` (modificado — atualizado p/ a nova UI: rota Recorrentes, abas, filtro de inativos)

## Senior Developer Review (AI)

**Revisor:** HugoMMBrito · **Data:** 2026-07-14 · **Resultado:** Aprovado (0 CRITICAL)

Revisão adversarial dos 10 arquivos do File List (bate 1:1 com o `git status` — sem discrepância), das 2 ACs e das 5 Tasks. Gate re-executado do zero sob Node 22 (`nvm use 22`): `typecheck` ✓, `lint` ✓, `build` ✓, `test` → **392 passed / 42 files** (`--no-file-parallelism`).

- **AC1 (CRUD migra p/ o Planner):** IMPLEMENTADA. `RecurringPage` em `/planner/recurring`, item "Recorrentes" (`EventRepeatIcon`) no grupo Planner da Sidebar, `SettingsPage.tsx`/`.test.tsx` apagados (dir removido), rota `settings` → `PlaceholderPage`, `grep -rn "SettingsPage" frontend/src` → zero (reconfirmado).
- **AC2 (abas + filtro):** IMPLEMENTADA. `Tabs`/`Tab` do MUI com `aria-label`, filtragem client-side por `group` + `showInactive` num único `useRecurringTemplatesQuery()`, form segue a aba ativa como `recurrenceGroup`, `Switch` "Mostrar inativos" default `false`, título único na página.
- **Task audit:** todas as [x] verificadas contra o código; heading interno do manager de fato removido (Task 2.1), backend intacto (nenhum arquivo `backend/` no diff, Task 5.2).

### Achados

| Sev | Achado | Ação |
| --- | ------ | ---- |
| 🟡 MEDIUM | Completion Notes documentavam **385 passed** mas a suíte real roda **392 passed / 42 files** — desvio do guardrail "contagem real" (retro Epic 3 §1). | **Corrigido** — contagem atualizada para 392. |
| 🟢 LOW | O `<Box role="tabpanel">` (RecurringTemplateManager.tsx:170) não é associado à aba selecionada via `aria-controls`/`aria-labelledby`. | **Não alterado** — decisão de design já documentada nas Dev Notes (linha 93), `jest-axe` passa e espelha o padrão existente em `WeekDaySelector`; não é violação do baseline WCAG 2.2 AA. Registrado como observação. |

## Change Log

| Data       | Descrição                                                                                     |
| ---------- | --------------------------------------------------------------------------------------------- |
| 2026-07-14 | Code review (AI): gate re-executado (392 passed / 42 files); corrigida contagem de testes 385→392 nas Completion Notes (guardrail Epic 3 §1); achado LOW de a11y de tabpanel registrado como observação. Status → done. |
| 2026-07-14 | Story 11.2 implementada: gestão de recorrentes migrada de Configurações para o Planner (`/planner/recurring`), templates em abas por grupo (Semanal/Mensal/Anual) + filtro "mostrar inativos"; `SettingsPage` removida (settings volta a placeholder); E2E `recurring-templates.spec.ts` atualizado p/ a nova UI. Status → review. |
