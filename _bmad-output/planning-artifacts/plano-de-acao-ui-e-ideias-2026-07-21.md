...
- [x] **[PRD]** `bmad-prd` (*update*) — **concluído 2026-07-22** (commit `b64319a`): [`prd.md`](prds/prd-hmmb-bujo-2026-06-15/prd.md) reorganizado por collection (núcleo + collections + plataforma) com FRs novos (FR-1/2/3/6, FR-10–14, FR-15.5–15.6, refinos FR-4.14–4.16), NFRs 7–9, Anexo A (de-para) e itens anotados do backlog confirmados (relatórios médicos → FR-13.11; dashboard de indicadores → FR-6.6)
- [x] **[ARCH]** `bmad-create-architecture` (*update*) — **concluído 2026-07-22**: [`architecture.md` §3b](architecture.md) — AD-17 a AD-27 registram as 12 decisões roteadas (proposal §9) + #15/#23 e o componente compartilhado Hoje/Dashboard (D4); tabela de rastreabilidade §9→AD no topo da §3b; decisão do architect: fotos do foodLog **referenciadas** (não copiadas); granularidade da flag registrada como deferida ao Épico 10; Stack (§2) e Estrutura (§7.5) atualizadas
- [x] **[CE]** `bmad-create-epics-and-stories` — **concluído 2026-07-23**: [`epics.md`](epics.md) atualizado in-place — inventário renumerado (Anexo A) + Épicos 12–22 e Épico 10 ampliado com **86 stories** na ordem mestre; x.0 de UX em todos os 11 épicos com UI; DIR-1..15 embutidas; revisão party-mode + 4 métodos de elicitação aplicados. Decisões novas: aliases finos p/ queues legadas (até É18), promoção única a prod ≈É18 (prod fica no sistema atual até lá), **rename "cardápio"→"Index"** (PRD a atualizar via [IR]), pendência de PRD: **recuperação de senha antes do Épico 10**
- [x] **[IR]** `bmad-check-implementation-readiness` — **concluído 2026-07-23**: [`implementation-readiness-report-2026-07-23.md`](implementation-readiness-report-2026-07-23.md) — veredicto **READY** (cobertura 100% do escopo comprometido; 109/110 FRs, FR-15.4 = backlog intencional). 1 crítico (FR de recuperação de senha antes do Épico 10 → [PRD]), 4 maiores (refresh stories 10.1/10.2; AD leve p/ deltas M06–M10 ou aceite formal; NFR-8 do PRD desalinhado; rename cardápio→Index no PRD), 5 menores (ordem mestre no [SP]; gate ⛔ 21.10/21.11; split da 14.5; notas de spine; nuance de rollback no migration-plan — reconciliada no report). **Nada bloqueia o [SP] nem o Épico 12**
