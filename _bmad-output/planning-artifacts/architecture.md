...
sessionStatus: 'COMPLETO — MVP: T1–T16 resolvidos (AD-01 a AD-16), READY FOR IMPLEMENTATION em 2026-06-22. UPDATE 2026-07-22 (rito [ARCH] do handoff do CC): AD-17 a AD-27 registram as 12 decisões roteadas pelo §9 do sprint-change-proposal-2026-07-22.md (pós-MVP: manifest, C5, refinos do núcleo, Journalling, home compartilhada, C6, Alimentação, IA global, Análises a/b/c, Pressão Arterial). UPDATE 2026-07-23 (rito [ARCH] pontual roteado pelo [IR] 2026-07-23, achado 🟠): AD-28 registra a modelagem dos deltas M06–M10 do Épico 14 (ciclo de vida Weekly/Monthly em colunas + ritual_decisions + fila unificada com aliases finos).'
inputDocumentsUpdate2026_07_23:
...
| _(adendo 2026-07-23 — roteado pelo [IR], achado 🟠 "Deltas M06–M10 sem AD dedicado")_ ciclo de vida Weekly/Monthly + decisões-snapshot + fila unificada (Épico 14) | AD-28 |
...
### AD-28 — Deltas de domínio dos spines M06–M10: ciclo de vida de Weekly/Monthly em colunas, decisões-snapshot de ritual, fila unificada de migração (Épico 14 — Onda 3, gate vertical)

**Contexto:** o EXPERIENCE.md 2026-07-17 ("Decisions for Architecture and Stories", M06/M07/M10) exige que os deltas de domínio dos spines passem por arquitetura antes da implementação. O CC 2026-07-22 aprovou o lado de produto e as stories 14.1–14.3 especificam gates e constraints — mas o [IR] 2026-07-23 (achado 🟠) apontou que a **forma** da modelagem não tinha AD, com risco de decisão ad-hoc no caminho crítico. Estado atual do código: `WeeklyLog`/`MonthlyLog` **não têm coluna de estado** — fechamento é 100% derivado on-read (`bujo/services/archive.py`: conteúdo + nenhuma `pending`/`started`); as filas de migração são duas views separadas (`/migration/queue/` para ontem, `/catch-up/queue/` para mês/semana/dia). As decisões de produto são vinculantes; aqui se decide só a forma. Convenções §6 aplicam-se integralmente.

**Decisões:**

1. **Estado do ciclo em colunas nos próprios logs — sem tabela de ciclo.** `WeeklyLog`/`MonthlyLog` já **são** o ciclo (1:1 por construção — `(user, week_start)`/`(user, month_first)`, AD-05); uma tabela satélite 1:1 só adicionaria join, dupla escrita e uma segunda superfície de constraint. Colunas idênticas nas duas tabelas: `status VARCHAR NULL` (`TextChoices`: `planning | active | finalized` + `CheckConstraint`, §6.1) e `planning_completed_at TIMESTAMPTZ NULL` — o marco "planejamento concluído" é **timestamp, não booleano** (registra quando; não congela o ritual, que continua revisitável até Iniciar). **`status IS NULL` = fora do regime operacional** — é a terceira semântica que a story 14.1 exige (monthlies futuros usados só como storage do Future Log; ciclos legados pré-regime), sem estado zumbi nem enum "none".
2. **Unicidade no banco, não na disciplina:** `UniqueConstraint` **parcial** por tabela — `(user_id) WHERE status = 'active'` e `(user_id) WHERE status = 'planning'` — garante no máximo UM "Em andamento" e UM "Em planejamento" por tipo. Corrida que viole a constraint → `IntegrityError` capturado no service → `DomainError` (409, §6.4). Um `active` + um `planning` simultâneos é o estado normal do método (planejar a próxima com a atual em andamento).
3. **Transições só por service explícito — materialização nunca atribui estado.** Services idempotentes (re-execução = no-op) com assinatura fixa e `@transaction.atomic` (§6.2): concluir planejamento / iniciar / finalizar, por tipo. Gates no service (nunca em serializer — §6.6): **Iniciar** exige `today_for(user) ≥` alvo + `planning_completed_at` preenchido + anterior finalizado (só `pending`/`started` do anterior bloqueiam); **Finalizar** é irreversível — `finalized` é terminal, sem transição de saída (matriz no service, padrão AD-02); **Concluir planejamento** é não-bloqueante. **Cancelar o alvo de planejamento** é a única transição "para trás": reverte `status` a `NULL` (o log sobrevive se já tiver registros). Os `get_or_create_*_log` existentes (captura, storage do Future Log, navegação) **nunca** atribuem estado — entrar no regime operacional é sempre ato de ritual. Meses pulados: materialização **sequencial**, um ciclo por vez, repetindo o mesmo fluxo (sem lote, sem fechamento automático); a janela regular mensal deriva da semana seg→dom que contém a virada, computada em `core/calendar.py` (helpers já existentes — §7).
4. **Data migration retroativa (bootstrap — a única exceção à regra 3):** reusa a derivação vigente de `services/archive.py` como critério. Fechados (conteúdo + nada aberto) → `finalized`; o ciclo do período corrente (por `today_for`) → `active`, **materializado dentro da migration se ausente** (logs são on-demand — o corrente pode não existir); monthlies futuros (storage do Future Log) e **ciclos passados não-fechados** → `NULL` — estes últimos ficam fora do regime e suas tarefas abertas são exatamente a população da fila unificada (item 7), que as escoa sem precisar de estado no ciclo. Contagens por bucket verificadas e registradas (AC da 14.1); migration nomeada (§6.1) e aplicada à branch Neon e2e antes do Playwright (lição recorrente 7.1/7.2).
5. **Arquivo (consumido pela story 14.10):** `finalized` passa a ser a fonte autoritativa de "ciclo fechado"; ciclos `NULL` legados que a derivação atual considera fechados continuam entrando pela derivação existente (fallback read-only) — zero perda de histórico, sem backfill especulativo de estados que nunca passaram pelos gates.
6. **Decisões-snapshot dos rituais: tabela própria `ritual_decisions`, um registro por (ritual-alvo × item) — só para decisões que NÃO mutam o item** ("Manter", "Não alocar nesta semana", "Manter sem dia"). Decisões mutantes (migrar/alocar/concluir/cancelar) **não** ganham registro paralelo: a própria mutação é a persistência — a fonte deixa de listar o item na re-derivação; registrar em dobro criaria segunda verdade. Shape: âncora **exclusiva** do alvo (`weekly_log` XOR `monthly_log`) e do item (`task` XOR `recurring_template` — "Não alocar nesta semana" decide sobre um **template**, não uma Task) — padrão CHECK exactly-one da AD-03/AD-20. `decision` em `TextChoices` + `CheckConstraint` (`keep | skip_week | keep_undated`); valores novos entram por migration limpa (§6.1). **Unicidade `(alvo, item)`** via constraints únicas parciais por combinação; re-decidir = upsert no service. **Persistência imediata, um POST por decisão** (pausar/sair não perde nada); **progresso do ritual é derivado** (elegíveis da fonte − mutados − decididos), nunca contador armazenado; a Task jamais é tocada por uma decisão-snapshot.
7. **Fila unificada de migração: um service derivado, zero schema novo.** `unified_migration_queue(*, user)` em `bujo/services/migration.py` (ao lado de `migrate_task` — a AD-09 já mora lá): derivação **por query** (sem cron, sem estado acumulado — filosofia AD-09 mantida) das raízes `pending`/`started` em períodos anteriores ao operacional, seções ordenadas **mês → semana → dia** ("ontem" = nível dia), cada uma com rótulo e contagem por fonte. Endpoint único novo: `GET /api/migration/unified-queue/` (§6.1). Decisão por item = mutação via os services existentes (`migrate_task`/`transition_task`), com herança de status e `waiting_on` **reusando a função de regra da AD-18** (sem duplicação — AC da 14.3); retomar = re-derivar (só os restantes, **sem posição salva** — AC explícito). Subárvores migram juntas (AD-08 item 11).
8. **Aliases finos (decisão (a) do party-mode 2026-07-22):** `MigrationQueueView` e `CatchUpQueueView` passam a **projetar a resposta do service unificado nos contratos de resposta atuais** — mesmas rotas, mesmos serializers, zero lógica própria de query. Testes de caracterização congelam os contratos; `MigrationBanner`/`CatchUpBanner` do Daily legado seguem intocados (premissa blindada: Daily legado plenamente utilizável até o Épico 17). Remoção dos aliases (rotas + serializers) no Épico 18, junto com os consumidores.

**Schema (delta sobre AD-05; uma migration nomeada por story):**
...
-- story 14.3: SEM schema novo (fila 100% derivada por query)
```

**Casos-âncora:**
- *Bootstrap (data migration):* semana corrente → `active` (materializada se ausente); semanas/meses fechados pela derivação atual → `finalized`; monthly de outubro usado só como storage do Future Log → `NULL`; semana velha com 2 tarefas abertas → `NULL` e as 2 tarefas aparecem na fila unificada.
- *Segundo "Planejar próxima semana" com um alvo já em planejamento:* constraint parcial bloqueia → 409; um alvo de planejamento por vez, por tipo.
- *Iniciar semana na sexta-feira anterior ao alvo:* `today_for(user) < alvo` → `InvalidTransition` (409); na segunda-feira, com planejamento concluído e anterior finalizado, passa — re-executar Iniciar depois é no-op (idempotência).
...
- *Fila unificada — tarefa `started` de semana antiga migrada para hoje:* sucessor nasce `started` e herda `waiting_on` (AD-18, mesma função de regra); a re-derivação remove o item; o `CatchUpBanner` legado (alias) devolve exatamente o contrato antigo.
...
**Adendo as-built 2026-07-25 (Story 14.1 entregue).** A AD acima decidiu a forma do schema e dos services, mas cinco detalhes de contrato só ficaram determinados na implementação. Registrados aqui para que as Stories 14.5/14.6/14.10 (UI) **não precisem re-derivá-los**:

1. **API do ciclo — um endpoint de ação por tipo.** `POST /api/bujo/logs/weekly/cycle/` e `POST /api/bujo/logs/monthly/cycle/`, com campo `action` despachado para o service correspondente (espelha `tasks/<pk>/transition/`, que já recebe `to_status`): mantém a superfície de URL e o diff de OpenAPI mínimos. `action` weekly = `open_planning_target | complete_planning | start | finalize | cancel_planning_target`; monthly = as mesmas **menos** `cancel_planning_target` (M07 não herda a ação — a ausência é regra de produto, e o endpoint a rejeita como 400). `open_planning_target` do monthly **ignora** `month_first`: o alvo é determinístico. Resposta: `{weekStart|monthFirst, status, planningCompletedAt}`, e no monthly mais `regularWindowStart`/`regularWindowEnd` (janela da virada, **informativa** — nenhum service a usa como pré-condição).
2. **`GET /logs/weekly/` e `GET /logs/monthly/` ganham `status` e `planningCompletedAt` como campos estritamente aditivos** — `closed` permanece com o mesmo nome e tipo. Os GETs leem o estado direto do log: não chamam service de ciclo e não atribuem estado (materialização nunca entra no regime).
3. **`closed` passa a ter UM critério por regime.** `is_cycle_closed(log)` é a autoridade consumida por `_check_container_open` e por `list_closed_cycles`: ciclo **dentro** do regime (`status` não-`NULL`) fecha se e somente se `status == finalized`; ciclo **fora** do regime (`status IS NULL`, legado do Épico 4) continua fechando pela derivação `is_container_closed`, que fica **intacta**. Os dois motivos: (a) a derivação exige `total_tasks > 0`, então um ciclo **`finalized` e vazio** era devolvido como aberto — ficava mutável e fora do Arquivo; (b) na direção oposta, aplicar a derivação a um ciclo **dentro** do regime fecharia um alvo `planning` (ou o `active`) no instante em que sua última tarefa fosse disposta, contra M06/M07 ("Um Weekly Em planejamento permite criar, editar, reordenar…"; "a data do calendário não finaliza uma semana automaticamente") — corrigido na code review da 14.1, com regressão em `test_services.py`/`test_views.py`. Como o backfill deixou `NULL` exatamente os ciclos que a derivação governava, nenhum ciclo pré-existente muda de resposta. `/archive/` devolve a união dos dois ramos (mutuamente exclusivos, sem duplicata). Consequência: **`reorder_task` entra no guardrail de ciclo fechado** junto de `create_task`/`update_task`/`delete_task` (antes não passava por ele).
4. **`CycleTargetConflict(DomainError)`** (novo em `core/exceptions.py`) é a tradução da colisão das uniques parciais → 409. Deliberadamente **distinta** de `InvalidTransition`: não é transição ilegal, é disputa de alvo perdida por um pedido que era legal quando foi lido. Todo erro de **gate** continua sendo `InvalidTransition`, como manda o item 3.
5. **Bootstrap de usuário novo e precedência do backfill.** (a) O alvo de `open_monthly_planning_target` é, em ordem: o `planning` já existente (no máximo um, pela unique parcial — é o que preserva a idempotência na janela entre `finalize` do mês corrente e `start` do planejado, em que não há `active`); senão o mês seguinte ao `active`; senão — conta que nunca entrou no regime, caso que os spines não cobrem — o **mês corrente** por `today_for(user)`, decisão interina coerente com o Weekly, que aceita a semana corrente por regra própria. A precedência do `planning` foi acrescentada na code review da 14.1. (b) Na data migration, **futuro vence a derivação de fechado**: um ciclo futuro sai como `NULL` mesmo com todas as tarefas dispostas — ele nunca esteve no regime, então não pode nascer `finalized`. A ordem é normativa: corrente → `active`; futuro → `NULL`; passado fechado → `finalized`; resto → `NULL`. (c) "Anterior" no gate de Iniciar é o ciclo **operacional** (`status` não-`NULL`) imediatamente anterior; ciclos `NULL` passados são ignorados — sem isso, todo usuário com semanas velhas abertas ficaria travado no primeiro uso real (o "ciclo órfão" que o item 4 proíbe).
...
_Adendo 2026-07-23 (rito [ARCH] pontual, roteado pelo achado 🟠 do [IR] 2026-07-23): **AD-28** registra a forma da modelagem dos deltas de domínio M06–M10 do Épico 14 — ciclo de vida de Weekly/Monthly em colunas nos próprios logs, `ritual_decisions` para decisões-snapshot, fila unificada derivada com aliases finos._
...
### Atualização 2026-07-23 — rito [ARCH] pontual (roteado pelo [IR] 2026-07-23)

- **AD-28 adicionada à Seção 3b:** fecha o achado 🟠 do `implementation-readiness-report-2026-07-23.md` ("Deltas M06–M10 sem AD dedicado") — a forma da modelagem dos deltas de domínio do Épico 14 (stories 14.1–14.3) está registrada: estado do ciclo em colunas de `weekly_log`/`monthly_log` (com `NULL` = fora do regime operacional), constraints únicas parciais para os gates de unicidade, `ritual_decisions` só para decisões não-mutantes, fila unificada 100% derivada com aliases finos até o Épico 18. Nenhuma decisão de produto alterada.
- **Estado:** Épico 14 (gate vertical, caminho crítico) sem lacuna arquitetural registrada; a modelagem deixa de depender de decisão ad-hoc no dev-story.
