...
function navigationSheet(page: Page): Locator {
  return page.getByRole('navigation', { name: 'Navegação completa' })
}

/** A nav principal (sidebar do desktop/tablet), onde vive a âncora de captura. */
function mainNav(page: Page): Locator {
  return page.getByRole('navigation', { name: 'Navegação principal' })
}

/** O paper do sheet de navegação (contém a nav "Navegação completa"). */
function sheetPaper(page: Page): Locator {
  return page
 ...
 * ambíguo.
 */
function sheetModalRoot(page: Page): Locator {
  return page
...
function captureSheet(page: Page): Locator {
  return page.getByRole('dialog', { name: 'Captura rápida' })
}

/**
 * Espera o slide de entrada/saída do sheet ASSENTAR. O `toBeFocused` do foco
 * inicial passa cedo (o `autoFocus` dispara na montagem, antes do `onEntered`),
 * então qualquer medida de geometria tirada logo após a abertura pega o paper
 * ainda transladado. O fim da transição é o `transform` neutro.
 */
