...
test('edita título e eisenhower via painel compartilhado em Semana e Mês (AC2)', async ({ page }) => {
  ...
  const weeklyTitleInput = weeklyPanel.getByLabel('Título')
  await weeklyTitleInput.fill('Editada na semana')
  await weeklyTitleInput.blur()
  ...
  const monthlyTitleInput = monthlyPanel.getByLabel('Título')
  await monthlyTitleInput.fill('Editada no mês')
  await monthlyTitleInput.blur()
  await page.getByLabel('Fechar').click()
