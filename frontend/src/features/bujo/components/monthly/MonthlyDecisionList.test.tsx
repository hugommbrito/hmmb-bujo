...
// `?raw` (mesma prova estrutural do irmão `weekly/WeeklyDecisionList.test.tsx`,
// DW-16): o `Button` do MUI sem `color` explícito herda o teal de marca legado
// do tema, abaixo de AA sobre `--ds-surface`. Cobre TODOS os botões do arquivo
// — inclusive os que só aparecem em erro por item, em erro da fonte e nos dois
// buckets fora do progresso — e pega qualquer botão novo.
describe('MonthlyDecisionList — cor explícita AA em TODO botão (DW-16)', () => {
  it('todo `<Button` do arquivo declara uma cor EXPLÍCITA de token', () => {
    ...
    const source = monthlyDecisionListSource.replaceAll('=>', '⇒')
  ...
  it('DECISION_BUTTON_SX declara a cor pelo token --ds-primary', () => {
    ...
    const declaration = monthlyDecisionListSource.match(/const DECISION_BUTTON_SX = [\s\S]*?\} as const/)?.[0]
